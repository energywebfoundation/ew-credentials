...
export class S3CredentialResolver implements CredentialResolver {
  ...
  private DID_STORE_RESOLVE_TIMEOUT = 60000;
  // NOTE: decrease this if receive `too many p2p requests
  private DID_STORE_BATCH_SIZE = 10;
  ...
  private async resolveFromDidStore(service: string): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      }, this.DID_STORE_RESOLVE_TIMEOUT);
  ...
  private async resolveFromDidStoreBatch(services: string[]): Promise<string[]> {
    const limit = plimit(this.DID_STORE_BATCH_SIZE);
